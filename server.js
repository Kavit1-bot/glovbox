...
app.get('/api/documents/export-pdf/:reg', authenticateToken, (req, res) => {
  ...
  res.setHeader('Content-Disposition', `attachment; filename=service-history-${req.params.reg}.pdf`);
  
  doc.pipe(res);
  
  doc.fontSize(20).text('Service History', { align: 'center' });
  doc.moveDown();
  doc.fontSize(12).text(`${vehicle.make} ${vehicle.model}`);
  ...
  if (records.length === 0) {
    ...
    records.forEach(r => {
      ...
      doc.text(`Type: ${r.type}`);
      if (r.mileage) doc.text(`Mileage: ${r.mileage}`);
      if (r.cost) doc.text(`Cost: £${r.cost}`);
      doc.moveDown();
    });
  }
